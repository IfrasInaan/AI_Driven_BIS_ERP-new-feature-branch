...
try:
    with open(get_model_path("Purchase_Demand_TotalCost_model.pkl"), "rb") as f:
        data_processor = pickle.load(f)

    lstm_model = load_model(get_model_path("Purchase_Demand_TotalCost_model.h5"), custom_objects={'mse': MeanSquaredError()})
except Exception as e:
    raise RuntimeError(f"Error loading models: {str(e)}")

def predict_purchase_cost(month, product_name):
    try:
