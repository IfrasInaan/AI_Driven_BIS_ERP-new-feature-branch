...
try:
    with open(get_model_path("Customer_Demand_model.pkl"), "rb") as f:
        data_processor = pickle.load(f)

    lstm_model = load_model(get_model_path("Customer_Demand_model.h5"), custom_objects={'mse': MeanSquaredError()})
except Exception as e:
    raise RuntimeError(f"Error loading models: {str(e)}")

def predict_demand(month, customer_name):
    try:
        month_num = pd.to_datetime(month).month
        features = np.array([[month_num, customer_name]])
